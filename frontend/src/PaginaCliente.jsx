...
export default function PaginaCliente() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/public/profissional/${id_profissional}`).then(res => res.json()).then(data => setDadosProfissional(data)).catch(err => console.error(err));
  ...
  const finalizarAgendamento = async () => {
    try {
      await fetch(`${API_URL}/public/agendamentos`, {
        ...
        body: JSON.stringify({ 
          ...
          data_reserva: dataEscolhida, horario: horarioEscolhido, valor: valorTotal
        })
      });
    ...
    } catch (error) { console.error(error); alert("Houve um erro ao realizar a reserva."); }
  };

  const podeContinuar = nome.trim() !== '' && whatsapp.trim().length >= 10;
  const lidarComVoltar = () => { if (etapa > 1) { setEtapa(etapa - 1); } else { window.history.back(); } };
    ...
    <div className="min-h-screen bg-[#080808] flex justify-center text-white font-['Inter']">
      <div className="w-full max-w-md bg-[#0D0D0D] min-h-screen relative flex flex-col shadow-[0_0_50px_rgba(0,0,0,0.5)]">
        <header className="p-6 text-center border-b border-[#2A2A2A] bg-[#0D0D0D]/90 backdrop-blur-md sticky top-0 z-10">
          <button onClick={lidarComVoltar} className="absolute left-6 top-7 text-[#A8A8A8] hover:text-[#D4AF37] transition-colors flex items-center gap-2 text-sm tracking-wide"><ArrowLeft size={16} strokeWidth={1.5} /> {etapa > 1 ? 'Voltar' : 'Sair'}</button>
        ...
        <main className="flex-1 p-6 w-full flex flex-col justify-start animate-fade-in pb-36 relative">
          ...
          {etapa === 1 && (
            <div className="space-y-10 animate-slide-up mt-8">
              <div className="text-center space-y-3"><h2 className="text-4xl font-normal font-['Playfair_Display'] text-white">Seja bem-vindo(a).</h2><p className="text-[#A8A8A8] font-light tracking-wide">Por favor, insira seus dados para a reserva.</p></div>
            ...
            <div className="space-y-6 animate-slide-up mt-4 h-full relative">
              ...
              <div className="space-y-4">
                {servicos.length === 0 ? <p className="text-[#A8A8A8] text-center text-sm py-4">Nenhum serviço cadastrado.</p> : servicos.map((servico) => {
                  ...
                  return (
                    <button key={servico.id} onClick={() => toggleServico(servico)} className={`w-full text-left p-6 transition-all rounded-xl group flex flex-col justify-center relative overflow-hidden ${isSelecionado ? 'bg-[#1A1A1A] border-2 border-[#D4AF37] shadow-[0_0_20px_rgba(212,175,55,0.15)]' : 'bg-[#1A1A1A] border-2 border-[#2A2A2A] hover:border-[#6F6F6F]'}`}>
                      ...
                      <div className={`flex justify-between items-center w-full ${isUltimo && !isSelecionado ? 'mt-4' : ''}`}>
                        <div><h3 className={`text-lg font-light transition-colors duration-300 ${isSelecionado ? 'text-[#D4AF37]' : 'text-white group-hover:text-[#D4AF37]'}`}>{servico.nome}</h3><p className="text-sm text-[#A8A8A8] mt-2 flex items-center gap-2"><Clock size={14} className={isSelecionado ? "text-[#D4AF37]/70" : "text-[#6F6F6F]"} /> {servico.tempo}</p></div>
                        <div className="flex items-center gap-4"><span className={`${isSelecionado ? "text-[#D4AF37]" : "text-white"} font-['Playfair_Display'] text-xl`}>R$ {parseFloat(servico.preco).toFixed(2).replace('.', ',')}</span><div className={`w-6 h-6 rounded-full border flex items-center justify-center transition-all ${isSelecionado ? 'bg-[#D4AF37] border-[#D4AF37]' : 'border-[#6F6F6F]'}`}>{isSelecionado && <CheckCircle2 size={14} className="text-[#0D0D0D]" />}</div></div>
                      </div>
                    </button>
                  );
                })}
              ...
              <div className="bg-[#1A1A1A] rounded-2xl p-6 border border-[#2A2A2A] mb-8 shadow-lg">
                 ...
                 <div className="grid grid-cols-3 gap-3">
                   {horarios.map((hora) => {
                     ...
                     const isSelecionado = horarioEscolhido === hora;

                     if (isOcupado) {
                       return (
                         <button key={hora} disabled className="py-3 rounded-xl text-sm font-medium border bg-[#0D0D0D] border-[#1A1A1A] text-[#4A4A4A] cursor-not-allowed line-through">
                           {hora}
                         </button>
                       );
                     }
                     ...
                     return (
                       <button key={hora} onClick={() => setHorarioEscolhido(hora)} className={`py-3 rounded-xl text-sm font-medium transition-all border ${isSelecionado ? 'bg-[#D4AF37] border-[#D4AF37] text-[#0D0D0D] shadow-[0_0_15px_rgba(212,175,55,0.3)]' : 'bg-[#0D0D0D] border-[#2A2A2A] text-[#A8A8A8] hover:border-[#6F6F6F] hover:text-white'}`}>{hora}</button>
                     );
                   })}
                 </div>
              </div>
