...
export default function ClienteHistoricoModal({
  ...
  const totalVisitas =
    listaHistorico.length > 0
      ...
      : Number(cliente?.total_visitas || 0);

  const totalGastoHistorico = listaHistorico.reduce((acc, item) => {
  ...
  const totalGasto =
    totalGastoHistorico > 0
      ? totalGastoHistorico
      : Number(cliente?.total_gasto || cliente?.valor_total || 0);
  ...
  const ultimaVisitaData =
    listaHistorico.length > 0
      ? obterDataItem(listaHistorico[0])
      : normalizarData(cliente?.ultima_visita);

  const ultimaVisita = ultimaVisitaData
    ? ultimaVisitaData.toLocaleDateString('pt-BR')
    : 'Sem visitas';

  return (
    <div className="fixed inset-0 z-999 bg-black/80 backdrop-blur-sm flex justify-center items-center p-4 animate-fade-in">
      <div className="bg-linear-to-br from-[#1A1A1A] to-[#0D0D0D] border border-[#2A2A2A] rounded-4xl w-full max-w-5xl max-h-[92vh] overflow-hidden shadow-[0_20px_80px_rgba(0,0,0,0.65)] animate-slide-up relative">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 p-6 border-b border-[#2A2A2A] relative z-10">
          <div className="bg-[#0D0D0D] border border-[#2A2A2A] rounded-3xl p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[#8A8A8A] text-[10px] uppercase tracking-widest">Visitas</span>
              <CalendarDays size={18} className="text-[#D4AF37]" />
            </div>
            <h3 className="text-white text-3xl font-bold">{totalVisitas}</h3>
          </div>

          <div className="bg-[#0D0D0D] border border-[#2A2A2A] rounded-3xl p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[#8A8A8A] text-[10px] uppercase tracking-widest">Total gasto</span>
              <Wallet size={18} className="text-[#D4AF37]" />
            </div>
            <h3 className="text-[#D4AF37] text-2xl font-['Playfair_Display']">{formatarMoeda(totalGasto)}</h3>
          </div>

          <div className="bg-[#0D0D0D] border border-[#2A2A2A] rounded-3xl p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[#8A8A8A] text-[10px] uppercase tracking-widest">Ticket médio</span>
              <TrendingUp size={18} className="text-[#D4AF37]" />
            </div>
            <h3 className="text-white text-2xl font-bold">{formatarMoeda(ticketMedio)}</h3>
          </div>

          <div className="bg-[#0D0D0D] border border-[#2A2A2A] rounded-3xl p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[#8A8A8A] text-[10px] uppercase tracking-widest">Última visita</span>
              <Clock3 size={18} className="text-[#D4AF37]" />
            </div>
            <h3 className="text-white text-lg font-bold">{ultimaVisita}</h3>
          </div>
        </div>

        <div className="overflow-y-auto max-h-[55vh] p-6 space-y-5 relative z-10">
          {listaHistorico.length === 0 ? (
            <div className="border border-dashed border-[#2A2A2A] rounded-4xl p-14 text-center bg-[#111]">
              <Crown size={42} className="text-[#D4AF37] mx-auto mb-5 opacity-70" />
              <h3 className="text-white text-2xl font-['Playfair_Display'] mb-3">Nenhum histórico detalhado encontrado</h3>
              <p className="text-[#8A8A8A] text-sm">
                O resumo do cliente já aparece acima. Os atendimentos detalhados aparecem aqui quando forem carregados pelo painel.
              </p>
            </div>
          ) : (
            listaHistorico.map((item, index) => {
              ...
              return (
                <div key={item.id || index} className="relative pl-8">
                  <div className="absolute left-2.5 top-0 bottom-0 w-0.5 bg-[#2A2A2A]"></div>
                  ...
                  <div className="bg-[#111] border border-[#2A2A2A] hover:border-[#D4AF37]/30 rounded-4xl p-6 transition-all duration-300">
                    <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-5">
                      <div className="space-y-3">
                        <div>
                          <h3 className="text-white text-xl font-bold">{item.servico_nome || 'Serviço'}</h3>
                          <p className="text-[#8A8A8A] text-sm mt-1">
                            Status: <span className="text-white">{item.status || 'pendente'}</span>
                          </p>
                        </div>

                        <div className="flex flex-wrap gap-3">
                          <span className="bg-[#0D0D0D] border border-[#2A2A2A] text-white px-4 py-2 rounded-full text-xs">
                            👤 {item.funcionario_nome || 'Equipe'}
                          </span>
                          <span className="bg-[#0D0D0D] border border-[#2A2A2A] text-[#D4AF37] px-4 py-2 rounded-full text-xs">
                            💰 {formatarMoeda(item.valor)}
                          </span>
                          <span className="bg-[#0D0D0D] border border-[#2A2A2A] text-white px-4 py-2 rounded-full text-xs">
                            ⏰ {item.horario || '--:--'} {item.horario_fim ? `às ${item.horario_fim}` : ''}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="text-right">
                        <p className="text-[#D4AF37] text-lg font-['Playfair_Display']">
                          {data ? data.toLocaleDateString('pt-BR') : 'Data não disponível'}
                        </p>
