...
// ==========================================
// 💎 PAINEL PROTEGIDO (V2.0 - SISTEMA COMPLETO)
// ==========================================
function PainelProfissional({ token, usuario, onLogout }) {
  ...
  const carregarTudo = useCallback(async () => {
    try {
      ...
      fetch(`${API_URL}/dashboard`, { headers: headersAPI }).then(r=>r.json()).then(d=>{ setGanhoDia(d.ganhoDia||0); setQtdAtendimentos(d.qtdAtendimentos||0); setGrafico(d.grafico7Dias||[]); });
      fetch(`${API_URL}/vendas`, { headers: headersAPI }).then(r=>r.json()).then(d=>setHistoricoCaixa(Array.isArray(d)?d:[]));
      fetch(`${API_URL}/despesas`, { headers: headersAPI }).then(r=>r.json()).then(d=>setDespesas(Array.isArray(d)?d:[]));
      ...
      if (usuario.is_ceo) { fetch(`${API_URL}/ceo/dashboard`, { headers: headersAPI }).then(r=>r.json()).then(setDadosCeo); }
  ...
  const concluirAtendimento = async (ag) => { await fetch(`${API_URL}/agendamentos/${ag.id}/concluir`, { method: 'POST', headers: headersAPI }); carregarTudo(); window.open(`https://wa.me/55${ag.cliente_whatsapp?.replace(/\D/g, '')}?text=Olá! Passando para agradecer sua visita hoje no ${usuario.nome}. Volte sempre!`, '_blank'); };
  ...
  // Cálculos do Caixa (Lucro Líquido)
  const totalEntradasHoje = historicoCaixa.filter(v => new Date(v.data_venda).toLocaleDateString() === new Date().toLocaleDateString()).reduce((acc, curr) => acc + parseFloat(curr.valor), 0);
  const totalComissoesHoje = historicoCaixa.filter(v => new Date(v.data_venda).toLocaleDateString() === new Date().toLocaleDateString()).reduce((acc, curr) => acc + parseFloat(curr.comissao_valor || 0), 0);
  const totalDespesasHoje = despesas.filter(d => new Date(d.data_criacao).toLocaleDateString() === new Date().toLocaleDateString()).reduce((acc, curr) => acc + parseFloat(curr.valor), 0);
  const lucroLiquidoHoje = totalEntradasHoje - totalComissoesHoje - totalDespesasHoje;

  // Lógica do Gráfico de Barras CSS
  const maxValorGrafico = Math.max(...grafico.map(g => g.valor), 1); // Evita divisão por zero
      ...
      <main className="flex-1 p-6 pb-36 space-y-8 overflow-y-auto">
          ...
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="bg-[#1A1A1A] p-6 rounded-3xl border border-[#2A2A2A]">
              ...
              <div className="flex items-end justify-between h-40 gap-2">
                {grafico.map((dia, i) => {
                  const altura = (dia.valor / maxValorGrafico) * 100;
                  return (
                    <div key={i} className="flex flex-col items-center flex-1 group">
                      <div className="w-full bg-[#0D0D0D] rounded-t-md relative flex items-end justify-center h-full">
                         <div className="w-full bg-linear-to-t from-[#D4AF37]/20 to-[#D4AF37] rounded-t-md transition-all duration-1000" style={{ height: `${altura}%` }}></div>
                         <span className="absolute -top-6 text-[9px] text-[#A8A8A8] opacity-0 group-hover:opacity-100 transition-opacity">R${dia.valor}</span>
                      </div>
        ...
        {telaAtiva === 'caixa' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-linear-to-br from-[#1A1A1A] to-[#0D0D0D] p-6 rounded-3xl border border-[#D4AF37]/50 flex flex-col gap-4 shadow-[0_0_20px_rgba(212,175,55,0.1)]">
              ...
              <div className="flex justify-between items-center text-sm"><span className="text-red-400 flex items-center gap-1"><TrendingDown size={16}/> Saídas (Comissões + Despesas)</span> <span className="font-bold">- R$ {(totalComissoesHoje + totalDespesasHoje).toFixed(2)}</span></div>
            ...
            <div className="flex justify-between items-center mt-8 mb-4">
              <h3 className="text-xl font-['Playfair_Display']">Despesas Operacionais</h3>
              <button onClick={() => setMostrarFormDespesa(!mostrarFormDespesa)} className="text-[#D4AF37] text-sm uppercase flex items-center gap-1"><Plus size={16}/> Adicionar</button>
            </div>

            {mostrarFormDespesa && (
               <div className="bg-[#1A1A1A] p-6 rounded-3xl border border-red-900/30 mb-6 flex flex-col gap-4">
                 <input type="text" placeholder="Ex: Conta de Luz, Pomada..." value={novaDespesa.descricao} onChange={e=>setNovaDespesa({...novaDespesa, descricao: e.target.value})} className="w-full bg-[#0D0D0D] border border-[#2A2A2A] p-4 text-white rounded-xl" />
                 <input type="number" placeholder="Valor (R$)" value={novaDespesa.valor} onChange={e=>setNovaDespesa({...novaDespesa, valor: e.target.value})} className="w-full bg-[#0D0D0D] border border-[#2A2A2A] p-4 text-white rounded-xl" />
                 <button onClick={adicionarDespesa} className="w-full bg-red-900/40 text-red-200 border border-red-900/50 p-4 rounded-xl font-bold">Lançar Despesa</button>
               </div>
            )}

            <div className="space-y-3">
              {despesas.filter(d => new Date(d.data_criacao).toLocaleDateString() === new Date().toLocaleDateString()).length === 0 ? <p className="text-[#6F6F6F] text-center text-sm">Nenhuma despesa hoje.</p> : despesas.filter(d => new Date(d.data_criacao).toLocaleDateString() === new Date().toLocaleDateString()).map(d => (
                 <div key={d.id} className="bg-[#1A1A1A] border border-[#2A2A2A] p-4 rounded-2xl flex justify-between items-center">
                   <div className="flex items-center gap-3"><Receipt size={18} className="text-red-400"/> <span>{d.descricao}</span></div>
                   <div className="flex items-center gap-3"><span className="text-red-400 font-bold">- R$ {parseFloat(d.valor).toFixed(2)}</span><button onClick={()=>removerDespesa(d.id)} className="text-[#6F6F6F]"><Trash2 size={16}/></button></div>
                 </div>
              ))}
            </div>
        ...
        {telaAtiva === 'equipe' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-4">
              {funcionarios.length === 0 ? <p className="text-[#6F6F6F] text-center">Nenhum funcionário.</p> : funcionarios.map(func => (
        ...
        {telaAtiva === 'config' && (
          <div className="space-y-8 animate-fade-in">
             ...
             <div className="border-t border-[#2A2A2A] pt-6">
                ...
                <div className="space-y-3">
                  {servicos.length === 0 ? <p className="text-[#6F6F6F] text-center">Nenhum serviço.</p> : servicos.map(serv => (
                 ...
                 return (
                   <div key={emp.id} className="bg-[#1A1A1A] p-6 rounded-2xl border border-[#2A2A2A] space-y-4">
                     <div className="flex justify-between items-start">
                        <div><p className="text-xl font-bold">{emp.nome}</p><p className="text-xs text-[#6F6F6F]">{emp.telefone} | ID: {emp.id}</p></div>
