...
// ==========================================
// 💎 PAINEL PROTEGIDO (BLINDADO E LIMPADO)
// ==========================================
      ...
      <main className="flex-1 p-6 pb-36 space-y-8 overflow-y-auto">
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center flex-1 group">
                      <div className="w-full bg-[#0D0D0D] rounded-t-md relative flex items-end justify-center h-full">
                         ...
                         <span className="absolute -top-6 text-[9px] text-[#A8A8A8] opacity-0 group-hover:opacity-100 transition-opacity">R${valorDia}</span>
                      </div>
              ...
              {safeHistorico.length === 0 ? <p className="text-[#6F6F6F] text-center">Nenhuma movimentação hoje.</p> : safeHistorico.map((item) => (
                 <div key={item.id} className="bg-[#1A1A1A] border border-[#2A2A2A] p-5 rounded-2xl flex flex-col gap-2">
                   <div className="flex justify-between items-center">
                     <div><p className="text-sm font-medium text-white">Venda Confirmada</p><p className="text-xs text-[#A8A8A8]">{new Date(item?.data_venda || Date.now()).toLocaleDateString()}</p></div>
                     <span className="text-[#D4AF37] text-lg font-bold">+ R$ {Number(item?.valor || 0).toFixed(2)}</span>
                   </div>
                   {item?.funcionario_nome && <div className="text-xs text-[#A8A8A8] mt-2 border-t border-[#2A2A2A] pt-2">Profissional: {item.funcionario_nome} | Comissão: <span className="text-red-400">- R$ {Number(item?.comissao_valor || 0).toFixed(2)}</span></div>}
                 </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-6 animate-fade-in">
             ...
             {(Array.isArray(agendamentos)?agendamentos:[]).length === 0 ? <p className="text-[#6F6F6F] text-center py-8">Agenda livre.</p> : (Array.isArray(agendamentos)?agendamentos:[]).map(ag => {
                ...
                <div key={ag.id} className="bg-[#1A1A1A] rounded-3xl border border-[#2A2A2A] overflow-hidden">
                  ...
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-4"><div><p className="text-lg font-medium">{ag?.cliente_nome}</p><p className="text-[#A8A8A8] text-sm">{ag?.servico_nome}</p></div><span className="text-xl">R$ {Number(ag?.valor || 0).toFixed(2)}</span></div>
              ...
              {(Array.isArray(listaClientes)?listaClientes:[]).length === 0 ? <p className="text-[#6F6F6F] text-center">Nenhum cliente salvo.</p> : (Array.isArray(listaClientes)?listaClientes:[]).map((cliente, idx) => (
                <div key={idx} className="bg-[#1A1A1A] border border-[#2A2A2A] p-6 rounded-2xl">
                   <div className="flex justify-between items-start mb-4">
                     ...
                     <span className="bg-[#D4AF37]/10 text-[#D4AF37] px-2 py-1 rounded text-[10px] font-bold">{cliente?.total_visitas || 0} Visitas</span>
                   </div>
                   <div className="bg-[#0D0D0D] p-3 rounded-xl flex justify-between items-center text-xs">
                     ...
                     <a href={`https://wa.me/55${String(cliente?.whatsapp || '').replace(/\D/g, '')}?text=Olá ${String(cliente?.nome || '').split(' ')[0]}! Que tal agendar um horário?`} target="_blank" rel="noreferrer" className="text-[#D4AF37] font-bold uppercase">Promoção</a>
                   </div>
                </div>
              ))}
            </div>
        ...
        {telaAtiva === 'equipe' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-4">
              {(Array.isArray(funcionarios)?funcionarios:[]).length === 0 ? <p className="text-[#6F6F6F] text-center">Nenhum funcionário.</p> : (Array.isArray(funcionarios)?funcionarios:[]).map(func => (
                <div key={func.id} className="bg-[#1A1A1A] p-5 rounded-2xl flex justify-between items-center border border-[#2A2A2A]">
                  <div><p className="font-medium text-lg">{func?.nome}</p><p className="text-sm text-[#A8A8A8]">Comissão: <span className="text-[#D4AF37]">{func?.comissao || 0}%</span></p></div>
        ...
        {telaAtiva === 'config' && (
          <div className="space-y-8 animate-fade-in">
             ...
             <div className="border-t border-[#2A2A2A] pt-6">
                ...
                <div className="space-y-3">
                  {(Array.isArray(servicos)?servicos:[]).length === 0 ? <p className="text-[#6F6F6F] text-center">Nenhum serviço.</p> : (Array.isArray(servicos)?servicos:[]).map(serv => (
                    <div key={serv.id} className="bg-[#1A1A1A] border border-[#2A2A2A] p-4 rounded-2xl flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-4"><span className="text-[#D4AF37]">R$ {Number(serv?.preco || 0).toFixed(2)}</span><button onClick={() => removerServico(serv.id)} className="text-[#6F6F6F]"><Trash2 size={16}/></button></div>
                    </div>
                  ))}
                </div>
             </div>
             ...
             <div className="border-t border-[#2A2A2A] pt-6">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {todosHorarios.map(hora => {
                    const atende = (Array.isArray(meusHorarios)?meusHorarios:[]).includes(hora);
        ...
        {telaAtiva === 'ceo' && usuario?.is_ceo && (
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#1A1A1A] p-6 rounded-3xl border border-[#2A2A2A]"><span className="text-xs text-[#A8A8A8] block">Salões Ativos</span><span className="text-3xl font-bold">{dadosCeo?.totalEmpresas || 0}</span></div>
              <div className="bg-[#1A1A1A] p-6 rounded-3xl border border-[#D4AF37]/30"><span className="text-xs text-[#A8A8A8] block">Giro Global</span><span className="text-2xl font-bold text-[#E6C76B]">R$ {Number(dadosCeo?.faturamentoGlobal || 0).toFixed(2)}</span></div>
            </div>
            <div className="space-y-4">
              {(Array.isArray(dadosCeo?.empresas)?dadosCeo.empresas:[]).map((emp) => {
                 const venc = new Date(emp?.data_vencimento || Date.now());
                 ...
                 return (
                   <div key={emp.id} className="bg-[#1A1A1A] p-6 rounded-2xl border border-[#2A2A2A] space-y-4">
                     ...
                     <div className="flex justify-between items-center text-xs text-[#A8A8A8] bg-[#0D0D0D] p-3 rounded-xl">
                        ...
                        <span>Total: <strong className="text-[#D4AF37]">R$ {Number(emp?.faturamento_total || 0).toFixed(2)}</strong></span>
                     </div>
