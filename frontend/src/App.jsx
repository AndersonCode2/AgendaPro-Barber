      ...
      {modalPagamento && (
        <div className="fixed inset-0 bg-black/90 flex justify-center items-center z-50 p-4 animate-fade-in">
          <div className="bg-[#1A1A1A] w-full max-w-xl rounded-[2.5rem] p-8 border border-[#2A2A2A] shadow-2xl flex flex-col items-center relative">
            <button onClick={onLogout} className="absolute top-6 right-6 text-[#A8A8A8] hover:text-white flex items-center gap-2 text-xs uppercase tracking-widest font-bold"> Sair <LogOut size={14}/></button>
            ...
            {!qrCodeBase64 ? (<button onClick={gerarPix} disabled={carregandoPix} className="w-full bg-linear-to-r from-[#D4AF37] to-[#E6C76B] p-6 rounded-3xl flex justify-between items-center mt-6 shadow-xl"><span className="text-[#0D0D0D] font-bold">AURUM PREMIUM - 30 DIAS</span><span className="text-2xl font-bold text-[#0D0D0D]">R$ 24,99</span></button>) : (<div className="bg-[#0D0D0D] p-8 rounded-3xl border border-[#D4AF37] flex flex-col items-center mt-6"><img src={`data:image/jpeg;base64,${qrCodeBase64}`} alt="Pix" className="w-48 h-48 mb-6" /><button onClick={() => { navigator.clipboard.writeText(qrCodeCopiaCola); alert('Copiado!'); }} className="w-full bg-[#1A1A1A] text-white py-4 rounded-xl flex justify-center gap-2"><QrCode size={18}/> Copiar Pix</button></div>)}
      ...
      <main className="flex-1 p-6 pb-36 space-y-8 overflow-y-auto">
        {telaAtiva === 'home' && (
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="bg-[#1A1A1A] p-6 rounded-3xl border border-[#2A2A2A]"><div className="flex items-center gap-2 mb-6"><TrendingUp size={18} className="text-[#D4AF37]"/><h3 className="text-lg font-['Playfair_Display']">Faturamento (7 Dias)</h3></div><div className="flex items-end justify-between h-40 gap-2">{safeGrafico.map((dia, i) => { const valorDia = Number(dia?.valor) || 0; const altura = maxValorGrafico > 0 ? (valorDia / maxValorGrafico) * 100 : 0; return (<div key={i} className="flex flex-col items-center flex-1 group"><div className="w-full bg-[#0D0D0D] rounded-t-md relative flex items-end justify-center h-full"><div className="w-full bg-linear-to-t from-[#D4AF37]/20 to-[#D4AF37] rounded-t-md transition-all duration-1000" style={{ height: `${altura}%` }}></div><span className="absolute -top-6 text-[9px] text-[#A8A8A8] opacity-0 group-hover:opacity-100 transition-opacity">R${valorDia}</span></div><span className="text-[10px] text-[#6F6F6F] mt-2">{dia?.data || ''}</span></div>); })}</div></div>
